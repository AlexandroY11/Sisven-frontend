                ...
                <form @submit.prevent="updateCustomer">
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="input-group">
                            ...
                            v-model="customer.document_number" disabled required>
                        </div>
                    </div>
...
export default { 
    ...
    methods:{
        ...
        async updateCustomer(){
            try {
                const res = await axios.put(`http://localhost:8000/api/customers/${this.customer.id}`, this.customer);
                if (res.status == 200) {
                    this.$router.push({name: 'Customers'});
                    Swal.fire({
                        position: 'top-end',
                ...
                console.error('Error actualizando la customer:', error);
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'No se pudo actualizar la customer. Por favor, intente de nuevo.',
                });
            }
    ...
    mounted(){
        ...
        axios.get(`http://localhost:8000/api/customers/${this.customer.id}`)
            .then(response => {
